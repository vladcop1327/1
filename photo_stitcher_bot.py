...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Выберите направление сшивки:", reply_markup=markup)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == WAITING_DIRECTION:
        ...
        await update.message.reply_text("Отправьте 1 фото:", reply_markup=ReplyKeyboardRemove())
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current < total:
        await update.message.reply_text(f"Фото {current} получено. Отправьте еще фото {current + 1}:")
    else:
        user_state[user_id] = WAITING_DESCRIPTION
        await update.message.reply_text("Введите описание для коллажа:")
